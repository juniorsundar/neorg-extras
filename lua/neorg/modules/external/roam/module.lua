	...
	node = function()
		...
		elseif module.config.public.fuzzy_finder == "Snacks" then
			local get_nodes = function()
				...
				for _, line in pairs(norg_files_output) do
					...
					if metadata ~= nil then
						table.insert(
							items,
							{ text = metadata["title"], file = full_path, preview = { text = full_path } }
						)
					else
						table.insert(items, { text = full_path, file = full_path, preview = { text = full_path } })
					end
				end
				return items
			end
			...
			module.private.snacks_modules.picker.pick({
				...
				actions = {
					insert_node_at_cursor = function(picker, item)
						picker:close()
						local current_file_path = item.file
						local escaped_base_path = base_directory:gsub("([^%w])", "%%%1")
						local relative_path = current_file_path:match("^" .. escaped_base_path .. "/(.+)%..+")
						vim.api.nvim_put({ "{:$/" .. relative_path .. ":}[" .. item.text .. "]" }, "", true, true)
					end,
					new_node = function(picker, item)
						local prompt = picker.finder.filter.pattern
						picker:close()
						local title_token = prompt
						if module.config.public.node_name_randomiser then
							title_token = name_tokeniser(prompt)
				...
				win = {
					input = {
						keys = {
							["<C-i>"] = {
								"insert_node_at_cursor",
								mode = { "n", "i" },
								desc = "Insert Node Reference at Cursor",
							},
							["<C-n>"] = { "new_node", mode = { "n", "i" }, desc = "Create New Node" },
						},
					},
				},
			})
			...
			local get_blocks = function()
				...
				for line in rg_results:gmatch("([^\n]+)") do
					...
					if metadata ~= nil then
						...
						table.insert(
							items,
							{ text = file .. " | " .. text, heading = text, file = file, pos = { tonumber(lineno), 1 } }
						)
					end
				end
				return items
			...
			module.private.snacks_modules.picker.pick({
				...
				actions = {
					insert_block_at_cursor = function(picker, item)
						...
						local heading_prefix = string.match(item.heading, "^(%** )")
						local heading_text = module.private.get_heading_text(filename, tonumber(item.pos[1]))
						if not heading_text then
							return
						else
							heading_text = heading_text:gsub("^%s+", "")
					...
					input = {
						keys = {
							["<C-i>"] = {
								...
								mode = { "n", "i" },
								desc = "Insert Block Reference at Cursor",
							},
						},
					},
				},
				...
				local get_backlinks = function()
					...
					for line in rg_results:gmatch("([^\n]+)") do
						...
						elseif metadata["title"] ~= self_title then
							table.insert(
								items,
								{ text = metadata["title"] .. " @" .. lineno, file = file, pos = { tonumber(lineno), 1 } }
							)
						else
							table.insert(
								items,
